...
Removed earnings calendar. Company deep-dive only.
"""
...
from utils.data import get_fmp, fmt_price, fmt_large
...
def render_kpi_row(info: dict):
    ...
    cols = st.columns(5)
    for i, (key, label, is_pct) in enumerate(metrics):
        val = info.get(key)
        val_s = f"{val*100:.2f}%" if (val and is_pct) else f"{val:.2f}" if val else "—"
        cols[i % 5].markdown(f"""
...
def render():
    st.markdown("""
        <div class="page-header">
            ...
            <span class="page-subtitle">Yahoo Finance · Financial Modeling Prep</span>
        </div>
    """, unsafe_allow_html=True)
    ...
    if not (ticker and load):
        st.info("Enter a ticker and click Load Fundamentals.")
        return
            ...
            <div style="display:flex;justify-content:space-between;align-items:flex-start">
                <div>
                    <div style="font-family:'IBM Plex Mono';font-size:1.3rem;
                                font-weight:600;color:#0353D9">{ticker}</div>
            ...
            <div class="bb-card" style="margin-top:0.5rem">
                ...
                <div style="display:flex;gap:2rem;font-family:'IBM Plex Mono'">
                    <div>
                        ...
                        <div style="font-size:1.2rem;color:#0353D9;font-weight:600">{rec}</div>
                    </div>
                    <div>
