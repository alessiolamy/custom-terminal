...
Positions stored in JSON file (data/portfolio.json).
Add/close trades via form. Live P&L, risk metrics, allocation chart.
"""
...
PORTFOLIO_FILE = os.path.join(os.path.dirname(os.path.dirname(__file__)),
                              "data", "portfolio.json")
...
def allocation_chart(df: pd.DataFrame):
    ...
    fig = go.Figure(go.Pie(
        ...
        hole=0.55,
        marker=dict(colors=["#0B4B99","#00CC66","#3399FF","#FFCC00","#FF3333","#AA44FF"]),
...
def render():
    ...
    with tab_dash:
        ...
        for col, label, val, is_pct in [
            (k1, "Portfolio Value",  f"${total_val:,.2f}", False),
            (k2, "Total P&L",        f"${total_pnl:+,.2f}", False),
            (k3, "Return %",         f"{total_pnl_pct:+.2f}%", False),
            (k4, "Today's P&L",      f"${day_pnl:+,.2f}", False),
        ]:
            color = "#00CC66" if (("+" in val or val.startswith("$0")) and val != "$0.00") \
                    else "#FF3333" if "-" in val else "#E0E0E0"
            col.markdown(f"""
        ...
        display = display[["Ticker","Name","Asset","Qty","Avg Cost","Current",
                            "Mkt Value","P&L","P&L %","Day Chg %"]]
    ...
    with tab_add:
        ...
        c1, c2 = st.columns(2)
        with c1:
            tk     = st.text_input("Ticker *", placeholder="e.g. AAPL, ASML.AS, BTC-USD").upper().strip()
            name   = st.text_input("Name / Description", placeholder="e.g. Apple Inc.")
            qty    = st.number_input("Quantity *", min_value=0.0001, step=1.0, value=1.0)
        with c2:
        ...
        notes = st.text_input("Notes (optional)", placeholder="e.g. Long-term hold, hedge")
        ...
        if st.button("✅ Add Position", type="primary"):
            if not tk:
                ...
                st.success(f"✓ Added {qty} × {tk} @ ${avg_price:.2f}")
                st.rerun()
        ...
        if st.session_state.portfolio:
            ...
            to_remove  = st.selectbox("Select position to remove", tickers_in)
            if st.button("🗑️ Remove", type="secondary"):
                st.session_state.portfolio = [
                    p for p in st.session_state.portfolio
                    if p["ticker"] != to_remove
                ]
                save_portfolio(st.session_state.portfolio)
